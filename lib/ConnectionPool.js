...
export class ConnectionPool {
    ...
    createDiceDBSocket() {
        ...
        const onConnectionTerminate = () => {
            if (socket.connecting) {
                return;
            }

            this.logger.warn(
                `Socket ${socket.socket_id} closed, trying to reconnect`,
            );
            ...
            socket
                ...
                .then(() => {
                    this.logger.success(
                        `Socket ${socket.socket_id} reconnected to the server, ${this.#pool.length} sockets remaining`,
                    );
                })
                .catch((err) => {
                    this.logger.error(
                        `Failed to reconnect socket ${socket.socket_id}: ${err}, removing socket from pool`,
                    );
        ...
        socket.on('end', onConnectionTerminate);
        socket.on('close', onConnectionTerminate);

        socket.on('error', (err) => {
            this.logger.error(
                `Removing socket ${socket.socket_id} from pool: ${err}`,
            );
        ...
        socket.on('timeout', () => {
            this.logger.warn(
                `Socket ${socket.socket_id} timed out, destroying socket, ${this.#pool.length} sockets remaining`,
            );
