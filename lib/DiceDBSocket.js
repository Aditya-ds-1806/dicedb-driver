...
export class DiceDBSocket extends EventEmitter {
    ...
    async connect() {
        return new Promise((resolve, reject) => {
            ...
            this.conn.once('error', errorCallback);
            ...
            this.conn.once('connect', () => {
                ...
                this.conn.off('error', errorCallback);
            });
            ...
            this.conn.once('ready', async () => {
                ...
                try {
                    ...
                    this.conn.on('end', (err) => this.emit('end', err));
                    this.conn.on('close', (err) => this.emit('close', err));
                    this.conn.on('error', (err) => this.emit('error', err));
                    this.conn.on('timeout', (err) => this.emit('timeout', err));
            ...
            this.conn.connect({ host: this.host, port: this.port });
        });
    }
